...
$CONFIG_FILE = './modules/companies/config.php';
...
$config_options = array(
    ...
    'description_max_length' => array(
        'description' => $AppUI->_('Maximum Description Length'),
        'value' => '65,535',
        'type' => 'text'
    ),
);
    ...
    if (is_writable($CONFIG_FILE)) {
        ...
        if (fwrite($handle, "<?php // Do not write to this file by hand, it will be overwritten by the configuration utility. \n") === FALSE) {
            ...
            foreach ($config_options as $key=>$value) {
                ...
                fwrite($handle, "\$COMPANIES_CONFIG['" . $key . "'] = '" . $val . "';\n");
            }
            fwrite($handle, "?>\n");
...
foreach ($config_options as $key=>$value)
    if (isset($TASKS_CONFIG[$key]))
        $config_options[$key]['value'] = $TASKS_CONFIG[$key];

// Setup the title block
$titleBlock = new CTitleBlock('Companies Module Configuration', 'handshake.png', $m, "$m.$a");
